...
def filter_linear_data(C, word_freq, word, c):
    print('Filtering V using linear comparison...')
    ...
    for i in range(len(word_freq)):
        ...
        linear_c.append([word_freq[i][1], float(angle[0][0])])
        d.write(word_freq[i][1] + '\t' + str(angle[0][0]) + '\n')

    linear_c.sort(key=lambda x: x[1])

    linear_c.reverse()

    for i in range(500):
        filtered_c.append([linear_c[i][0]])
        if (linear_c[i][0] == word):
            word_in_rank = True

    d.close()

    if (word_in_rank):
        print('Tatget word in filtered set of c')
        print('Filtered set c of length: ' + str(len(filtered_c)))
        return filtered_c
    else:
        print('Target word was not found in filtered set of c. Reverting c.')
        return c
